...
2. Admin/공개 리포트 Supabase 기준 조회 확인
   ...
   - 다음에는 실제 브라우저에서 Admin 날짜 드롭다운/미리보기 iframe/공개 `/report` 첫 화면이 `2026-05-21`을 표시하는지 눈으로 확인한다.
3. 검증 pre-upload gate 운영 확인
   ...
   - 현재 Codex 세션에서는 Python/`requests` 실행 환경이 없어 `scripts\Validate-DailyData.ps1`, `scripts\Check-ExcelCoverage.ps1`가 끝까지 돌지 못했다. 운영 PC에서는 `.venv-docling` 복구 또는 `DAILY_REPORT_PYTHON` 지정이 먼저 필요하다.
4. 승인 UI dogfooding
