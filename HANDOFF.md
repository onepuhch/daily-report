...
- `scripts\Refresh-InfomaxWorkbook.ps1`에 인포맥스 사전 체크를 추가했다.
  - 기본 필수 프로세스: `infomaxmain`, `imxlcommapp`.
  - 둘 중 하나가 없으면 엑셀을 열기 전에 실패 처리한다.
  - 실패 메시지는 “Infomax program is not running...”으로 남고, 운영자는 인포맥스를 먼저 실행한 뒤 재실행해야 한다.
  - 필요하면 `.env`의 `INFOMAX_REQUIRED_PROCESSES`에 쉼표 구분 프로세스명을 넣어 기준을 조정할 수 있다.
- 최신성 경고를 보강했다.
...
1. Admin 자동화 로그 dogfooding
   ...
   - 인포맥스 프로그램이 꺼져 있으면 Excel add-in이 자동 동작하지 않는 조건을 발견했고, 배치 시작 전 `infomaxmain`, `imxlcommapp` 프로세스 체크를 추가했다.
...
07:00 배치가 성공 코드로 끝났지만 최신 리포트가 `2026-05-20`에 머문 것을 확인. 인포맥스 프로그램이 꺼져 있으면 Excel add-in이 동작하지 않는 조건을 반영해 `infomaxmain`, `imxlcommapp` 사전 체크를 추가하고, 최신 생성일이 요청 종료일보다 오래되면 상태 점검/Admin 로그 요약에서 경고하도록 보강.
