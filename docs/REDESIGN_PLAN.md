...
상세 내용은 같은 디렉터리의 다른 파일들 참고:

- **`docs/STATUS.md`** — 현재 상태 스냅샷 (항상 최신)
- **`docs/REDESIGN_LOG.md`** — 시간순 작업 일지 (append-only)
- **`docs/DECISIONS.md`** — 의사결정 기록 (ADR 스타일)
- **`AGENTS.md`** "현재 진행 중" 섹션 — 새 세션 빠른 진입점

### 핵심 원칙

**토큰이 갑자기 끊겨도 다음 도구가 즉시 이어할 수 있어야 한다.**

- 작업 **시작 전**에 무엇을 할지 LOG에 먼저 적는다 (의도 기록)
- 작업 **중간**에도 의미 있는 변경 단위마다 갱신 (30분~1시간 단위)
- 작업 **종료 시점에만** 몰아서 기록하지 않는다
- 가능한 한 자주 `git commit` (push 못 해도 로컬 커밋은 남음)

### 세션 시작 체크리스트

1. `git pull` — 최신 동기화
2. `docs/STATUS.md` — 어디까지 됐고 다음 할 일 파악
3. `docs/REDESIGN_LOG.md` 최상단 항목 — 직전 작업 맥락
4. `docs/DECISIONS.md` — 최근 결정 살피기
5. `AGENTS.md` "현재 진행 중" 확인
6. 작업 시작 전 REDESIGN_LOG.md에 새 항목(의도) 추가
7. 작업 진행
8. 변경 단위마다 LOG·STATUS 갱신 + 커밋
9. 가능 시 push
